package myproject.groupware.config;

import lombok.RequiredArgsConstructor;
import myproject.groupware.handler.AuthFailureHandler;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter { // Spring Security의 설정파일로서의 역할을 하기 위해 상속
    ...
    private final MemberService userService; // 유저 정보를 가져올 클래스
    ...
    public void configure(WebSecurity web) { // 인증을 무시할 경로, static 하위 파일 목록(css, js, img) 인증 무시
        web.ignoring().antMatchers("/css/**", "/js/**", "/img/**"); // h2-console 추가
    }
